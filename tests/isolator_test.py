...
from util import FeatureGroup, FeatureGroups, assure_path, load_csv, save_csv
...
def assert_isolate(extractor_result, isolated_data):
    ...
    assert (
        len(extractor_result.columns) == len(isolated_data_column_names)
...
def find_correlation_of_isolated_data(
    isolated_data, workload_features, energy_source=test_energy_source, power_columns=get_expected_power_columns()
):
...
def get_max_corr(
    ...
    corr_data = find_correlation_of_isolated_data(
        isolated_data, workload_features, energy_source=energy_source, power_columns=power_columns
    )
    try:
...
def process(
    test_isolators=test_isolators,
    customize_isolators=[],
    extract_path=extractor_output_path,
    save_path=isolator_output_path,
):
    ...
    for test_instance in test_isolators:
        ...
        for extractor in test_extractors:
            ...
            for feature_group, extract_result in extractor_results.items():
                ...
                isolated_data = test_instance.isolate(
                    extract_result, label_cols=get_expected_power_columns(), energy_source=test_energy_source
                )
