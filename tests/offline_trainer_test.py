...
from util.loader import class_to_json, list_all_abs_models, list_all_dyn_models
...
def process(
    ...
    for isolator, isolator_args in isolators.items():
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        make_request(
            pipeline_name, idle_prom_response, isolator, isolator_args, train_prom_response, energy_source, save_path
        )
        assert_offline_trainer_output(
            target_path=target_path, energy_source=energy_source, valid_fgs=valid_fgs, pipeline_name=pipeline_name
        )
