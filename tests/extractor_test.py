...
from train import DefaultExtractor, SmoothExtractor, load_class
from util import (
    FeatureGroup,
    FeatureGroups,
    PowerSourceMap,
...
def assert_extract(extracted_data, power_columns, energy_components, num_of_unit, feature_group):
    ...
    assert (
        node_info_column in extracted_data_column_names
    ), f"no {node_info_column} in column {extracted_data_column_names}"
    ...
    # detail assert
    assert (
        len(power_columns) == expected_power_column_length
    ), f"unexpected power label columns {power_columns}, expected {expected_power_column_length}"
    # TODO: if ratio applied, expected_col_size must + 1 for power_ratio
    expected_col_size = (
        expected_power_column_length + len(FeatureGroups[FeatureGroup[feature_group]]) + num_of_unit
    )  # power ratio
    assert (
...
def process(
    ...
    for test_instance in test_extractors:
        extracted_data, power_columns, corr, _ = test_instance.extract(
            query_results, energy_components, feature_group, energy_source, node_level=True
        )
