...
from kepler_model.util.loader import base_model_url, load_remote_json, load_metadata
from kepler_model.util.saver import NODE_TYPE_INDEX_FILENAME
from kepler_model.util.config import download_path, default_pipelines
        ...
        for node_type, info in node_types.items():
            ...
            for cmp_node_type, cmp_info in node_types.items():
                ...
                if node_type == cmp_node_type:
                    ...
                    assert similarity >= 0, "similarity must be >= 0, {}-{} ({})".format(node_type, cmp_node_type, similarity)
                    assert similarity <= 1, "similarity must be <= 1, {}-{} ({})".format(node_type, cmp_node_type, similarity)
