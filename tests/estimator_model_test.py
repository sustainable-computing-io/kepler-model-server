...
from isolator_test import get_isolate_results, isolator_output_path, test_isolators
from train.trainer import model_toppath
from util import FeatureGroup, ModelOutputType, list_model_names
from util.loader import default_train_output_pipeline, get_model_group_path
...
def test_model(group_path, model_name, test_data_with_label, power_columns, power_range=None):
    ...
    for energy_component, _ in predicted_power_map.items():
        ...
        sum_power_label = (
            test_data_with_label.groupby([TIMESTAMP_COL]).mean()[label_power_columns].sum(axis=1).sort_index()
        )
...
def process_all(
    ...
    isolate_save_path=isolator_output_path,
    extract_save_path=extractor_output_path,
):
    abs_test_list = []
    dyn_test_list = []
    ...
    for extractor in extractors:
        ...
        for isolator in isolators:
            ...
            for feature_group, result in isolator_results.items():
                ...
                result_df = test_all_models(
                    result, power_columns, ModelOutputType.DynPower, FeatureGroup[feature_group]
                )
                result_df["extractor"] = extractor_name
...
if __name__ == "__main__":
    ...
    print(
        abs_train_df.set_index(["energy_source", "component", "extractor"])[focus_columns].sort_values(
            by=["mae"], ascending=True
        )
    )
    print("Container-level test results:")
    print(
        dyn_train_df.set_index(["energy_source", "component", "extractor", "isolator"])[focus_columns].sort_values(
            by=["mae"], ascending=True
        )
    )
