...
from profile import get_min_max_watt, profile_process
...
from prom_types import TIMESTAMP_COL, prom_responses_to_results
            ...
            for output_type in [ModelOutputType.AbsPower, ModelOutputType.DynPower]:
                ...
                for feature_group in list_subfolder(model_output_path):
                    ...
                    for model_path in model_paths:
                        ...
                        extracted_data, power_columns, _, _ = extractor.extract(
                            test_data, energy_components, feature_group, energy_source, node_level=False
                        )
                        ...
                        if not model.feature_check(feature_columns):
                            print(
                                f"model {model.name} ({energy_source}/{output_type.name}/{feature_group})is not valid to test"
                            )
                            continue
                        ...
                        for energy_component, values in predicted_data.items():
                            ...
                            sum_power_label = (
                                predicted_data.groupby([TIMESTAMP_COL])
                                .mean()[label_power_columns]
                                .sum(axis=1)
                                .sort_index()
                            )
                            ...
                            sum_predicted_power = (
                                data_with_prediction.groupby([TIMESTAMP_COL])
                                .sum()
                                .sort_index()[predicted_power_colname]
                            )
                            if output_type == ModelOutputType.AbsPower:
                                ...
                                item["profile_mae"], item["profile_mse"] = compute_error(
                                    sum_power_label, profile_reconstructed_power
                                )
                                ...
                                predicted_background_dynamic_power_values = predicted_background_dynamic_power[
                                    energy_component
                                ]
                                dynamic_background_power_colname = get_predicted_dynamic_background_power_colname(
                                    energy_component
                                )
                                background_data[dynamic_background_power_colname] = (
                                    predicted_background_dynamic_power_values
                                )
                                ...
                                item["train_bg_mae"], item["train_bg_mse"] = compute_error(
                                    sum_power_label, trained_reconstructed_power
                                )
                                ...
                                item["min_bg_mae"], item["min_bg_mse"] = compute_error(
                                    sum_power_label, min_reconstructed_power
                                )
                                item["min"] = min_watt
