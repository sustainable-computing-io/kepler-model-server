...
test-pipeline:
	$(CTR_CMD) run --platform linux/amd64 -i $(TEST_IMAGE) /bin/bash -c "python3.8 ./tests/pipeline_test.py"
...
run-collector-client:
	$(CTR_CMD) exec estimator /bin/bash -c "while [ ! -S "/tmp/estimator.sock" ]; do sleep 1; done; python3.8 ./tests/estimator_power_request_test.py"
...
run-estimator-client:
	$(CTR_CMD) exec model-server /bin/bash -c "python3.8 ./tests/estimator_model_request_test.py"
...
test: build-test test-pipeline test-estimator test-model-server