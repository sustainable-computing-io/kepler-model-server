...
from loader import get_archived_file, get_model_group_path, load_pipeline_metadata, load_weight
from saver import save_json
from train_types import FeatureGroup, ModelOutputType, PowerSourceMap, weight_support_trainers
...
class ExportModel:
    def __init__(self, models_path, output_type, feature_group, energy_source, pipeline_name, model_name, metadata):
        ...
        self.source_model_group_path = get_model_group_path(
            models_path, output_type, feature_group, energy_source, assure=False, pipeline_name=pipeline_name
        )
...
class BestModelCollection:
    ...
    def compare_new_item(self, export_item):
        current_best = self.collection[export_item.energy_source][export_item.output_type.name][
            export_item.feature_group.name
        ]
        ...
        if current_best is None or current_best.metadata[self.error_key] > compare_mape:
            self.collection[export_item.energy_source][export_item.output_type.name][export_item.feature_group.name] = (
                export_item
            )
            self.has_model = True
        if export_item.weight is not None:
            current_best = self.weight_collection[export_item.energy_source][export_item.output_type.name][
                export_item.feature_group.name
            ]
            if current_best is None or current_best.metadata[self.error_key] > compare_mape:
                self.weight_collection[export_item.energy_source][export_item.output_type.name][
                    export_item.feature_group.name
                ] = export_item
        ...
        for ot in ModelOutputType:
            ...
            for _, row in metadata_df.iterrows():
                ...
                if row["mape"] <= mape_threshold or row["mae"] <= mae_threshold:
                    ...
                    export_item = ExportModel(
                        models_path, ot, fg, energy_source, pipeline_name, model_name, row.to_dict()
                    )
