...
from saver import _pipeline_model_metadata_filename, _power_curve_filename, assure_path
...
{}
    """.format(
        ...
        if pipeline_metadata["abs_trainers"] == pipeline_metadata["dyn_trainers"]
        else pipeline_metadata["dyn_trainers"],
        workload_content,
    )
    ...
    for energy_source in collection.keys():
        ...
        for output_type_name in collection[energy_source].keys():
            ...
            for feature_group_name, best_item in collection[energy_source][output_type_name].items():
                best_item_with_weight = best_model_collection.get_best_item_with_weight(
                    energy_source, output_type_name, feature_group_name
                )
...
def generate_report_results(local_export_path, best_model_collections, node_type_index_json, remote_version_path):
    for node_type, collection in best_model_collections.items():
        if best_model_collections[int(node_type)].has_model:
            ...
            markdown_content += (
                data_to_markdown_table(pd.DataFrame([node_type_index_json[str(node_type)]["attrs"]])) + "\n"
            )
