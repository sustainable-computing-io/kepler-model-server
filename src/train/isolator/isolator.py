...
from extract_types import all_container_key, col_to_component, container_id_colname, get_num_of_unit
from prom_types import TIMESTAMP_COL, get_container_name_from_id, node_info_column
...
def exclude_target_container_usage(data, target_container_id):
    ...
    filled_target_container_data = (
        data[target_container_data.columns].join(target_container_data, lsuffix="_target").fillna(0)
    )
    filled_target_container_data.drop(
        columns=[col for col in filled_target_container_data.columns if "_target" in col], inplace=True
    )
...
def get_target_containers(data, background_containers):
    ...
    target_containers = [
        ...
        if get_container_name_from_id(container_id) not in background_containers
    ]
...
def squeeze_data(container_level_data, label_cols):
    ...
    feature_columns = [
        col
        for col in container_level_data.columns
        if col not in node_level_columns and col not in ratio_columns and col not in container_indexes
    ]
...
class ProfileBackgroundIsolator(Isolator):
    ...
    def isolate(self, data, label_cols, energy_source):
        ...
        try:
            for label_col in label_cols:
                ...
                isolated_data["profile"] = isolated_data[node_info_column].transform(
                    self.transform_profile, energy_source=energy_source, component=component
                )
    ...
    def reconstruct(self, extracted_data, data_with_prediction, energy_source, label_cols):
        ...
        for energy_component in energy_components:
            try:
                copy_data["profile"] = copy_data[node_info_column].transform(
                    self.transform_profile, energy_source=energy_source, component=energy_component
                )
