    ...
    def fit(self, X_values, y_values):
        ...
        if self.p0_func is not None:
            self.popt, self.pcov = curve_fit(
                self.fit_func, flatten_x, flatten_y, p0=self.p0_func(flatten_x, flatten_y), maxfev=30000
            )
        else:
...
class CurveFitTrainer(Trainer):
    def __init__(
        ...
        super(CurveFitTrainer, self).__init__(
            model_class,
            energy_components,
            feature_group,
            energy_source,
        ...
        for component, model in self.node_models[node_type].items():
            ...
            weight_dict[component] = {
                "All_Weights": {
                    ...
                    "Numerical_Variables": {
                        self.features[i]: {"scale": scaler.scale_[i]} for i in range(len(self.features))
                    },
