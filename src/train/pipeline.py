...
from config import ERROR_KEY, model_toppath
...
from loader import get_all_metadata, get_archived_file, get_metadata_df, get_pipeline_path
...
from train_types import FeatureGroups, ModelOutputType, PowerSourceMap


def load_class(module_name, class_name):
    path = os.path.join(os.path.dirname(__file__), f"{module_name}/{class_name}")
    sys.path.append(path)
    import importlib

    module_path = importlib.import_module(f"train.{module_name}.{class_name}.main")
...
class Pipeline:
    ...
    def get_abs_data(self, query_results, energy_components, feature_group, energy_source, aggr):
        extracted_data, power_labels, _, _ = self.extractor.extract(
            query_results, energy_components, feature_group, energy_source, node_level=True, aggr=aggr
        )
        return extracted_data, power_labels

    def get_dyn_data(self, query_results, energy_components, feature_group, energy_source):
        extracted_data, power_labels, _, _ = self.extractor.extract(
            query_results, energy_components, feature_group, energy_source, node_level=False
        )
    ...
    def prepare_data(self, input_query_results, energy_components, energy_source, feature_group, aggr=True):
        ...
        # 1. get abs_data
        extracted_data, power_labels = self.get_abs_data(
            query_results, energy_components, feature_group, energy_source, aggr=aggr
        )
    ...
    def prepare_data_from_input_list(
        self, input_query_results_list, energy_components, energy_source, feature_group, aggr=True
    ):
        index = 0
        abs_data_list = []
        dyn_data_list = []
        ...
        for input_query_results in input_query_results_list:
            extracted_data, isolated_data, extracted_labels = self.prepare_data(
                input_query_results, energy_components, energy_source, feature_group, aggr
            )
            if extracted_data is None:
    ...
    def _train(self, abs_data, dyn_data, power_labels, energy_source, feature_group):
        ...
        with ThreadPoolExecutor(len(self.trainers)) as executor:
            ...
            self.print_log(
                f"{len(futures)}/{len(self.trainers)} trainers are trained from {feature_group} to {energy_source}"
            )

    def process(
        self, input_query_results, energy_components, energy_source, feature_group, aggr=True, replace_node_type=None
    ):
        self.print_log(f"{feature_group} start processing.")
        abs_data, dyn_data, power_labels = self.prepare_data(
            input_query_results, energy_components, energy_source, feature_group, aggr
        )
    ...
    def process_multiple_query(
        ...
        feature_group,
        aggr=True,
        replace_node_type=None,
    ):
        abs_data, dyn_data, power_labels = self.prepare_data_from_input_list(
            input_query_results_list, energy_components, energy_source, feature_group, aggr
        )
    ...
    def print_pipeline_process_end(self, energy_source, feature_group, abs_data, dyn_data):
        ...
        if abs_data is not None:
            ...
            abs_metadata_df, abs_group_path = get_metadata_df(
                model_toppath, ModelOutputType.AbsPower.name, feature_group, energy_source, self.name
            )
            ...
            for node_type in node_types:
                ...
                min_mae = (
                    -1 if len(filtered_data) == 0 else filtered_data.loc[filtered_data[ERROR_KEY].idxmin()][ERROR_KEY]
                )
        ...
        if dyn_data is not None:
            ...
            dyn_metadata_df, dyn_group_path = get_metadata_df(
                model_toppath, ModelOutputType.DynPower.name, feature_group, energy_source, self.name
            )
            dyn_messages = [
        ...
        for feature_group in valid_feature_groups:
            for trainer_name in trainer_names:
                ...
                trainer = trainer_class(
                    energy_components, feature_group.name, energy_source, node_level, pipeline_name=pipeline_name
                )
                trainers += [trainer]
    return trainers
