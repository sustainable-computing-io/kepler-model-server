...
from prom_types import PROM_QUERY_INTERVAL, get_valid_feature_group_from_queries
    ...
    while True:
        ...
        for energy_source, energy_components in PowerSourceMap.items():
            for feature_group in valid_feature_groups:
                success, _, _ = profile_pipeline.process(
                ...
                if not success:
                    ...
                    success, _, _ = non_profile_pipeline.process(
                        query_results, energy_components, energy_source, feature_group=feature_group
                    )
                    if success:
