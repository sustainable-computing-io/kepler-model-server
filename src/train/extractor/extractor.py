...
from extract_types import accelerator_type_colname, component_to_col, container_id_colname, get_unit_vals, ratio_to_col
...
from train_types import SYSTEM_FEATURES, FeatureGroup, FeatureGroups
    ...
    def get_workload_feature_data(self, query_results, features):
        ...
        for feature in features:
            ...
            if all(col in aggr_query_data.columns for col in container_id_cols):
                ...
                aggr_query_data[container_id_colname] = aggr_query_data[container_id_cols].apply(
                    lambda x: "/".join([str(xi) for xi in x]), axis=1
                )
                ...
                for container_id in container_id_list:
                    ...
                    if len(container_df) > 1:
                        ...
                        if container_id in container_df_map:
                            ...
                            container_df_map[container_id] = pd.concat(
                                [container_df_map[container_id], feature_df], axis=1
                            )
                        else:
        ...
        for component in energy_components:
            ...
            if unit_col is not None:
                if usage_ratio_query not in query_results:
                    ...
                    aggr_query_data = (
                        aggr_query_data.groupby([unit_col, TIMESTAMP_COL]).sum().reset_index().set_index(TIMESTAMP_COL)
                    )
