...
_tmp_model_server_path = pathlib.Path("/tmp/model-server")
...
def _init_mnt_path() -> pathlib.Path:
    ...
    mnt_path = get_config("MNT_PATH", pathlib.Path("/mnt/model-server"))

    if not os.path.exists(mnt_path) or not os.access(mnt_path, os.W_OK):
        # use local path if not exists or cannot write
        tmp_mnt_path = _tmp_model_server_path / "mnt" / "model-server"
