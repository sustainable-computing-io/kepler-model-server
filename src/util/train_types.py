...
PowerSourceMap = {"intel_rapl": ["package", "core", "uncore", "dram"], "acpi": ["platform"]}
...
SingleSourceFeatures = [
    FeatureGroup.CounterOnly.name,
    FeatureGroup.CgroupOnly.name,
    FeatureGroup.BPFOnly.name,
...
class XGBoostMissingModelXOrModelDescException(Exception):
    ...
    def __init__(
        self, missing_model: bool, missing_model_desc: bool, message="model is missing xor model_description is missing"
    ) -> None:
...
EnergyComponentLabelGroups = {
    ...
    EnergyComponentLabelGroup.PackageDRAMEnergyComponents: deep_sort(
        PACKAGE_ENERGY_COMPONENT_LABEL + DRAM_ENERGY_COMPONENT_LABEL
    ),
}
