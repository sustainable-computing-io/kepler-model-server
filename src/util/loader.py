...
default_pipelines = {"intel_rapl": "ec2", "acpi": "specpower"}
base_model_url = f"https://raw.githubusercontent.com/sustainable-computing-io/kepler-model-db/main/models/v{version}"
...
def load_pkl(path, name):
    ...
    try:
        ...
        print(f"fail to load pkl {filepath}: {err}")
        return None
...
def is_matched_type(nodeCollection, spec, pipeline_name, model_name, node_type, energy_source):
    ...
    if node_type == any_node_type:
        ...
        if pipeline_name not in nodeCollection or nodeCollection[pipeline_name].node_type_index[model_node_type].cover(
            spec
        ):
            return True
    return False
...
def list_pipelines(model_toppath, energy_source, model_type):
    ...
    pipeline_names = [
        ...
        if os.path.exists(os.path.join(model_toppath, f, pipeline_metadata_filename + ".csv"))
    ]
...
def get_version_path(output_path, assure=True):
    version_path = os.path.join(output_path, f"v{version}")
    if assure:
