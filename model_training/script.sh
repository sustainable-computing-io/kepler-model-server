...
expect_num() {
	...
	shift 2
	kubectl get benchmark "$benchmark" -n "$benchmark_ns" -oyaml | tee tmp.yaml
...
save_benchmark() {
	...
	local benchmark_ns="$2"
	shift 2
	kubectl get benchmark "$benchmark" -n "$benchmark_ns" -ojson | tee "$DATAPATH/$benchmark.json"
}
...
collect_idle() {
	...
	if ! "$NATIVE"; then
		docker run --rm -v "$DATAPATH":/data --network=host --entrypoint kepler-model "$ENTRYPOINT_IMG" query "${args[@]}"
	else
		kepler-model query "${args[@]}" || true
	fi
}
...
collect_data() {
	...
	if ! "$NATIVE"; then
		docker run --rm -v "$DATAPATH":/data --network=host --entrypoint kepler-model "$ENTRYPOINT_IMG" query "${args[@]}" || true
	else
		kepler-model query "${args[@]}" || true
	fi
}
...
train_model() {
	...
	if ! "$NATIVE"; then
		...
		docker run --rm -v "$DATAPATH":/data --entrypoint kepler-model "$ENTRYPOINT_IMG" train "${args[@]}" || true
	else
		echo "Train natively"
		kepler-model train "${args[@]}" || true
	fi
}
...
validate() {
	...
	if ! "$NATIVE"; then
		docker run --rm -v "$DATAPATH":/data --entrypoint kepler-model "$ENTRYPOINT_IMG" validate "${args[@]}"
	else
		kepler-model validate "${args[@]}" || true
	fi
}
...
_export() {
	...
	echo "${args[@]}"
	if ! "$NATIVE"; then
		docker run --rm -v "$DATAPATH":/data -v "$output":/models --entrypoint kepler-model "$ENTRYPOINT_IMG" export "${args[@]}" -o /models
	else
		kepler-model export "${args[@]}" -o "$output"
	fi
	return 0
}
