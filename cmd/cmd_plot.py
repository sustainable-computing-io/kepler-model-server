...
from util.loader import get_model_group_path, load_metadata, load_scaler
...
def feature_power_plot(
    ...
    for xi in range(row_num):
        ...
        for yi in range(col_num):
            ...
            sns.lineplot(
                data=sorted_data, x=feature_col, y=predicted_power_cols[yi], ax=ax, label="predicted", color="C1"
            )
            if xi == 0:
...
def summary_plot(args, energy_source, summary_df, output_folder, name):
    ...
    for i in range(col_num):
        ...
        data = summary_df[
            (summary_df["energy_source"] == energy_source) & (summary_df["energy_component"] == component)
        ]
...
def metadata_plot(args, energy_source, metadata_df, output_folder, name):
    ...
    for i in range(col_num):
        ...
        sns.barplot(
            ...
            hue="trainer",
            ax=ax,
            hue_order=sorted(metadata_df["trainer"].unique()),
            errorbar=None,
            palette="Set3",
        )
        ax.set_title(component)
...
def _load_all_models(model_toppath, output_type, name, node_types, energy_source):
    ...
    for model_node_type in node_types:
        ...
        for trainer in weight_support_trainers:
            model, metadata, cpu_ms_max = _get_model(
                model_toppath, trainer, model_node_type, output_type=output_type, name=name, energy_source=energy_source
            )
            if metadata is None:
                continue
...
def _plot_models(
    models, cpu_ms_max, energy_source, output_folder, name, max_plot=15, cpu_time_bin_num=10, sample_num=20
):
    import numpy as np
    import pandas as pd
    import seaborn as sns
    ...
    predicted_col = {"acpi": "default_platform_power", "intel_rapl": "default_package_power"}
    ...
    for data_with_predictions in data_with_prediction_list:
        ...
        for data_with_prediction_index in data_with_predictions:
            ...
            sns.lineplot(
                data=data_with_prediction,
                x=main_feature_col,
                y=predicted_col[energy_source],
